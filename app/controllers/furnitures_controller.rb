class FurnituresController < ApplicationController
    ...
    def optionajax
        ...
        p params
        puts "/////////////////////"
        color_code = if JSON.parse(params[:color]) == "" then nil else JSON.parse(params[:color]) end
        capacity = if JSON.parse(params[:capacity]) == "" then nil else JSON.parse(params[:capacity]) end
        material_img = if JSON.parse(params[:material]) == "" then nil else JSON.parse(params[:material]) end
        kuan = if JSON.parse(params[:kuan]) == "" then nil else JSON.parse(params[:kuan]) end
        furniture_id = JSON.parse(params[:id])
        @furniture = Furniture.find(furniture_id).furniture_option.where(color_code: color_code, capacity: capacity,material_img: material_img,kuan: kuan)
